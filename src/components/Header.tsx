            ...
            <div className="px-6 pt-6 pb-6 space-y-2">
              {navItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="block w-full text-left px-4 py-4 text-slate-300 hover:text-blue-400 hover:bg-slate-800 rounded-lg transition-all duration-200 text-lg font-medium"
                >
                  {item.name}
                </button>
              ))}
